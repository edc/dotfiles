...
set -x GEM_HOME /usr/local

set -x PATH /usr/local/bin $PATH
#set -x PATH $PATH /usr/local/opt/coreutils/libexec/gnubin
set -x PATH $PATH ./node_modules/.bin/
test -d /usr/local/opt/go/libexec/bin; and set -x PATH $PATH /usr/local/opt/go/libexec/bin
set -x WERKZEUG_DEBUG_PIN off

if test -d $HOME/.go-env
  set -x GOPATH $HOME/.go-env
...
# quick options for common commands
set -x AG --ignore-dir tests --ignore-dir south_migrations --ignore tags --ignore tests.py --ignore 'fake*' --python
...
# virtualfish
set -g VIRTUALFISH_COMPAT_ALIASES # for virtualenvwrapper-style commands
...
alias lint "cd ~/w; make gitlint; cd -"
alias retest "python manage.py test --retest --run-all"
alias djtest "python manage.py test --run-all"
